...
# ---------------------------
TOKENS = [
 "8588986616:AAFnOI-LeVeatpk6YB03N1XsGLWHmeapxJI",
 "7928169142:AAHMQxvMw4iZ5wf9r1cJ8wC2_fBOuXvGM2Q",
 
 "7971421749:AAGtvu-XSu7msTy2379qxM6zVu0PbeW_au4",
 
 "8331671836:AAF88yRo4wOWRdLvrlNrT-dta1y5F7CkKog",
 
 "8483249321:AAFLbh-OhcYePEQGMDKZ0ZrjMf_-u7oFKOs",
]
...
RAID_TEXTS = [
    ...
    "चल तेरी माँ की चुत 🥵🥵", "Tera बाप 𝘚𝘞1𝘗𝘌𝘙| आरुष🌙"
]
...
def only_sudo(func):
    async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if uid not in SUDO_USERS:
            await update.message.reply_text("𝘛𝘔𝘒𝘊کنआरुष!🇬🇪ραρα кα вσт мтт छेड़.")
            return
...
def only_owner(func):
    async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE):
        uid = update.effective_user.id
        if uid != OWNER_ID:
            await update.message.reply_text("आरुष! ko baap bol mdc🌚🎀🧃")
            return
        return await func(update, context)
    return wrapper
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🪷 Zeno V4 Ultra Multi — Commands 🐼\nUse /help")

async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    help_text = """
🪷 आरुष...!! V4 Ultra Multi — Commands 🐼
...
async def stopncemo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id in group_tasks:
        for task in group_tasks[chat_id]:
            task.cancel()
        del group_tasks[chat_id]
        await update.message.reply_text("आरुष! PAPA CHORD DIA USKO🦋🍫🧃🎀")
    else:
...
async def run_all_bots():
    ...
    print(f"🎉 आरुष! V4 Ultra Multi is running with {len(bots)} bots!")
